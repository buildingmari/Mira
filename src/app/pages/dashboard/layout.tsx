...
export function DashboardLayout() {
  ...
  // Auth: read directly from sessionStorage — same pattern as MiraDashboard
  ...
  if (!authReady) {
    return (
      <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F8F9FB' }}>
        <div style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 22, background: 'linear-gradient(135deg,#2563EB,#3B82F6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>MIRA</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-background, #F8F9FB)', display: 'flex', fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <motion.div key="ov" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            style={{ position: 'fixed', inset: 0, zIndex: 299, background: 'rgba(0,0,0,.3)' }}
          />
        )}
      </AnimatePresence>
        ...
        <div style={{ borderTop: '1px solid rgba(0,0,0,0.07)' }}>
          <div style={{ padding: '10px 12px' }}>
            <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
              style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 10px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 14, background: 'transparent', color: '#6B7280', fontFamily: "'DM Sans',sans-serif" }}>
          ...
          <div style={{ padding: '14px 20px', borderTop: '1px solid rgba(0,0,0,0.07)', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontSize: 13, fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#111827', margin: 0 }}>{userName}</p>
      ...
      <nav className="lg:hidden" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          {MOBILE_NAV.slice(0, 2).map(({ path, label, Icon }) => (
            <button key={path} onClick={() => navigate(path)} style={mobileBtn(isActive(path))}>
