...
export function DashboardLayout() {
  ...
  const { userSession, logout, loading } = useUserSession();

  // While sessionStorage is being hydrated, show nothing (avoids flicker redirect)
  if (loading) {
    return (
      <div style={{ height: '100vh', background: '#F8F9FB', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ fontFamily: "'Sora',sans-serif", fontWeight: 700, fontSize: 24, background: 'linear-gradient(135deg,#2563EB,#3B82F6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>MIRA</div>
      </div>
    );
  }
  ...
  const navBtnStyle = (active: boolean): React.CSSProperties => ({
    ...
    color: active ? '#1D4ED8' : 'var(--color-muted-foreground)',
    transition: 'background .15s, color .15s', textAlign: 'left',
  });

  const mobileNavBtnStyle = (active: boolean): React.CSSProperties => ({
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-background)', display: 'flex', fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <motion.div key="ov" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            style={{ position: 'fixed', inset: 0, zIndex: 299, background: 'rgba(0,0,0,.3)', backdropFilter: 'blur(2px)' }}
          />
        )}
      </AnimatePresence>
      ...
      {/* ── Sidebar */}
      <aside style={{
        width: 220, background: 'var(--color-sidebar)', borderRight: '1px solid var(--color-sidebar-border)',
        position: 'fixed', top: 0, left: 0, bottom: 0, display: 'flex', flexDirection: 'column',
        zIndex: 300, transition: 'transform .28s cubic-bezier(.4,0,.2,1)',
        ...
        <div style={{ padding: '22px 20px 18px', borderBottom: '1px solid var(--color-sidebar-border)', display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden"
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, borderRadius: 6, color: 'var(--color-muted-foreground)', display: 'flex' }}>
            ...
            <div key={section.label} style={{ padding: '14px 12px 4px' }}>
              ...
              {section.items.map(({ path, label, Icon }) => (
                <button key={path} onClick={() => { navigate(path); setSidebarOpen(false); }}
                  style={navBtnStyle(isActive(path))}>
        ...
        <div style={{ borderTop: '1px solid var(--color-sidebar-border)' }}>
          <div style={{ padding: '10px 12px' }}>
            <button onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
              style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 10px', borderRadius: 8, border: 'none', cursor: 'pointer', fontSize: 14, background: 'transparent', color: 'var(--color-muted-foreground)', fontFamily: "'DM Sans',sans-serif" }}>
      ...
      {/* ── Main */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }} className="lg:ml-[220px]">

        {/* Desktop topbar */}
        <div className="hidden lg:flex" style={{
          position: 'sticky', top: 0, zIndex: 50,
          background: 'rgba(248,249,251,0.92)', backdropFilter: 'blur(16px)',
          borderBottom: '1px solid var(--color-border)',
          padding: '14px 32px', alignItems: 'center', justifyContent: 'space-between',
        }}>
          <div>
        ...
        <div className="lg:hidden" style={{
          ...
          background: 'rgba(248,249,251,0.96)', backdropFilter: 'blur(20px)',
          borderBottom: '1px solid var(--color-border)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <button onClick={() => setSidebarOpen(true)}
                style={{ background: 'none', border: 'none', width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: 'var(--color-foreground)' }}>
      ...
      <nav className="lg:hidden" style={{
        ...
        background: 'rgba(255,255,255,0.94)', backdropFilter: 'blur(24px) saturate(1.8)',
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          {MOBILE_NAV.slice(0, 2).map(({ path, label, Icon }) => (
            <button key={path} onClick={() => navigate(path)} style={mobileNavBtnStyle(isActive(path))}>
