...
type Step = 'idle' | 'checking' | 'verified' | 'paying' | 'done' | 'error';
...
export function WAPanel({
  ...
  // ── STEP 1: Daftar & verifikasi nomor ───────────────────────────
  const handleDaftar = async () => {
    ...
    // Nomor valid & belum terdaftar — simpan payload untuk step bayar
    setSavedPayload(payloadFinal);
    setStep('verified');
  };

  // ── STEP 2: Bayar sekarang ───────────────────────────────────────
  ...
  const inputLocked = isVerified || isBusy;

  // Label & handler tombol utama
  const btnLabel =
    ...
    <div id="wa-panel" className="show">
      ...
      {!isVerified && !isPaying && (
        <div style={{ background: 'var(--blue-ultra)', borderRadius: '10px', padding: '12px 14px', fontSize: '0.82rem', color: '#334155', marginTop: '4px', marginBottom: '20px' }}>
          ℹ️ MIRA akan menghubungi nomor-nomor ini via WhatsApp setelah pembayaran berhasil.
        </div>
      )}
